...
- **Display** (400, `text-balance`, leading 1.04–1.08): H1 do hero, três linhas separadas por `<br>` (2,1rem abaixo de 360px, 2,4rem de 360px, 3,05rem no `md`), e o H2 que nomeia o produto na virada, "Foi pra esse ano que existe o Plano Anual Biblinho." (2,4rem → 3,5rem). Os três degraus do H1 são calibrados pela frase mais longa do topo: cada linha do `<br>` precisa caber sozinha na coluna, senão o destaque verde quebra no meio. Ao reescrever o topo, medir de novo — a coluna do texto é `1.32fr` a partir de `xl`.
